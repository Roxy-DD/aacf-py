...
**对弱模型。** 任务已被分解成它能处理的简单步骤。弱模型通过遵循清晰的规则完成“不可能”的任务。

**对强模型。** 你通过 `.how()` 限定了分析维度。强模型的“自由决策”完全在你的掌控之中。

**对开发者。** 五元组 DSL 强制你先想清楚。代码的可维护性和可控性大幅提升。

**对系统。** 每一步都被显式规则约束。系统看起来动态且智能，但完全可预测、可解释、可维护。

### 为什么这比“更强的模型”更有价值

```
更强的模型  ->  更深的黑盒、更高的成本、更大的心智负担
更多的功能  ->  选择爆炸、文档追不上、用户迷茫
更好的规则设计 ->  弱模型变强、强模型变可控
```

这就是“阳谋”原则：模型相信自己在做出智能决策，而实际上所有决策都在你显式定义的空间中展开。不是牢笼——是棋盘。规则没有限制游戏；规则使游戏成为可能。
...
**为什么用五元组 DSL？** 强制对每个节点进行原子化思考。但更重要的是，它是“规则内化自由”的机制——模型相信自己在自由推理，而它思考的每个维度都已被开发者显式设计。不是通过压制来约束，而是通过结构来解放。
